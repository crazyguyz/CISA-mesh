        ...
        with self.write_lock:
            ...
            try:
                ...
                try:
                    ...
                    c.execute("SELECT asset_id, name FROM assets_inventory "
                              "WHERE computer_asset_id=? AND category='printer' AND source='auto'",
                              (computer_asset_id,))
                    for asset_id, aname in c.fetchall():
                        an = (aname or "").strip()
                        if an and an not in current_usb_names:
                            ...
                            c.execute("INSERT INTO assets_change_log (asset_id, asset_type, change_type, details) "
                                      "VALUES (?,'printer','printer_disconnected',?)",
                                      (asset_id, json.dumps(detail, ensure_ascii=False)))
                            changes.append({"type": "printer_disconnected", "asset_id": asset_id,
