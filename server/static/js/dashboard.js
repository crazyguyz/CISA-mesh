...
function renderTailscaleAuth(d) {
    ...
    if (st === 'ok') {
        cls = 'success';
        banner = t('au.aOk', [d.days_left, d.expires_at]);
    } else if (st === 'expiring' || st === 'expiring_critical') {
        cls = (st === 'expiring_critical') ? 'danger' : 'warning';
        banner = t('au.aExpiring', [d.days_left, d.expires_at]);
    } else if (st === 'expired') {
        cls = 'danger';
        banner = t('au.aExpired', [d.expires_at]);
    } else if (st === 'no_expiry') {
        cls = 'warning';
        banner = t('au.aNoExpiry');
    } else if (st === 'bad_expiry') {
        cls = 'warning';
        banner = t('au.aBadExpiry');
    } else if (st === 'not_configured') {
        cls = 'secondary';
        banner = t('au.aNotConfig');
    } else {
        cls = 'warning';
        banner = t('au.aKeyInvalid');
    }
    ...
    var daysTxt = (d.days_left != null)
        ? (d.days_left < 0 ? t('au.aOverdue', [Math.abs(d.days_left)]) : t('au.aLeftDays', [d.days_left]))
        : t('au.aNoDate');
    var html = '';
    ...
    html += '<div style="flex:1;min-width:200px;background:#111827;border:1px solid #1e2a3a;border-radius:8px;padding:10px;"><div style="font-size:10px;text-transform:uppercase;color:#8b9bb4;">' + escapeHtml(t('au.aCur')) + '</div><div style="font-family:monospace;font-size:13px;word-break:break-all;">' + (d.masked ? escapeHtml(d.masked) : '<i style="color:#8b9bb4;">' + escapeHtml(t('au.aNone')) + '</i>') + '</div><div style="font-size:10px;color:#8b9bb4;">' + escapeHtml(t('au.aCurHint')) + '</div></div>';
    html += '<div style="flex:1;min-width:130px;background:#111827;border:1px solid #1e2a3a;border-radius:8px;padding:10px;"><div style="font-size:10px;text-transform:uppercase;color:#8b9bb4;">' + escapeHtml(t('au.aExpires')) + '</div><div style="font-size:15px;">' + escapeHtml(d.expires_at || '—') + '</div><div style="font-size:11px;color:#8b9bb4;">' + escapeHtml(daysTxt) + '</div></div>';
    html += '<div style="flex:1;min-width:190px;background:#111827;border:1px solid #1e2a3a;border-radius:8px;padding:10px;"><div style="font-size:10px;text-transform:uppercase;color:#8b9bb4;">' + escapeHtml(t('au.aFlow')) + '</div><div style="font-size:12px;">' + escapeHtml(t('au.aFlowDesc')) + '</div></div>';
    html += '</div>';
    html += '<div style="background:#0f172a;border:1px solid #1e2a3a;border-radius:8px;padding:12px;">';
    html += '<div style="font-size:13px;margin-bottom:8px;"><b>' + escapeHtml(t('au.aUpdateTitle')) + '</b> <span style="color:#8b9bb4;font-size:11px;">' + escapeHtml(t('au.aUpdateSub')) + '</span></div>';
    html += '<div class="row g-2">';
    html += '<div class="col-md-8"><input type="text" class="form-control" id="tsAuthInput" placeholder="' + escapeHtml(t('au.aKeyPh')) + '" value="" style="background:var(--bg-dark);color:#d0d8e0;border-color:var(--border-color);" spellcheck="false"></div>';
    ...
    html += '<button class="btn btn-sm btn-primary me-2" onclick="saveTailscaleAuthkey()"><i class="bi bi-save"></i> ' + escapeHtml(t('au.aSave')) + '</button>';
    html += '<button class="btn btn-sm btn-outline-danger" onclick="clearTailscaleAuthkey()"><i class="bi bi-trash"></i> ' + escapeHtml(t('au.aClear')) + '</button>';
    html += '</div></div>';
    html += '<div style="margin-top:10px;font-size:11px;color:#8b9bb4;">' + escapeHtml(t('au.aNote')) + '</div>';
    el.innerHTML = html;
}
...
function loadTailscaleAuthkey() {
    ...
    fetch('/api/tailscale/authkey').then(function(r) {
        ...
        if (!res.ok) throw new Error(res.j.error || t('au.aLoadErr'));
        renderTailscaleAuth(res.j);
    }).catch(function(e) {
        body.innerHTML = '<div class="alert alert-danger py-2 mb-0" style="font-size:12px;">❌ ' + escapeHtml(t('au.aLoadErr')) + ': ' + escapeHtml(String(e.message || e)) + '</div>';
    });
}
...
function saveTailscaleAuthkey() {
    ...
    if (!key && !exp) { showToast(t('au.aNothing')); return; }
...
function loadUsers() {
    ...
    fetch('/api/users').then(function(r) { return r.json(); }).then(function(users) {
        ...
        html += names.map(function(u) {
            const usr = users[u] || {};
            const cur = usr.role === 'admin' ? 'admin' : (usr.role === 'operator' ? 'operator' : (usr.role === 'analyst' ? 'analyst' : 'viewer'));
            ...
            // v5.0.4 (Phase3 B11): analyst role = read + triage
            ['viewer','analyst','operator','admin'].forEach(function(r) {
...
// ===== v5.0.4 (Phase1 A2 + Phase2 A7): LOG SOURCE COVERAGE =====
function loadCoverage() {
    const el = document.getElementById('coverageList');
    if (!el) return;
    el.innerHTML = '<div class="text-center text-muted py-3"><i class="bi bi-hourglass-split"></i> ' + t('ui.loading') + '</div>';
    // Top Risk Hosts (A7) + coverage table
    fetch('/api/risk/hosts?since=168').then(r => r.json()).then(rd => {
        const hosts = (rd.hosts || []).slice(0, 5);
        let riskCard = '';
        if (hosts.length) {
            const bars = hosts.map(function(h) {
                const pct = Math.max(4, h.score);
                const color = h.score >= 70 ? '#ff6666' : h.score >= 40 ? '#ffcc66' : '#66cc99';
                return '<div style="margin-bottom:4px;"><div class="d-flex justify-content-between" style="font-size:10px;color:#8892a4;"><span>' + escapeHtml(h.hostname || h.machine_id) + '</span><span>' + h.score + ' / ' + h.alerts + ' alert</span></div>' +
                    '<div style="height:6px;background:#1e2a3a;border-radius:3px;"><div style="width:' + pct + '%;height:100%;background:' + color + ';border-radius:3px;"></div></div></div>';
            }).join('');
            riskCard = '<div style="background:#0f1923;border:1px solid #2a3a4a;border-radius:8px;padding:10px 12px;margin:8px;margin-bottom:0;"><strong style="color:#e4e7eb;font-size:12px;">🔥 Top Risk Hosts (7 ngày)</strong>' + bars + '</div>';
        }
        fetch('/api/health/coverage').then(r2 => r2.json()).then(d => {
            const machines = d.machines || [];
            if (!machines.length) { el.innerHTML = riskCard + '<div class="text-center text-muted py-3"><i class="bi bi-info-circle"></i> Chưa có máy trạm nào.</div>'; return; }
            ...
            el.innerHTML = riskCard + tableWrap(['Máy', 'Version', 'Trạng thái', 'Events 24h', 'TB 7 ngày', 'Độ sụt', 'Cảnh báo'], machines.map(m => {
                ...
                return '<tr><td><strong style="color:#e4e7eb;">' + escapeHtml(m.hostname) + '</strong><br><small style="font-family:monospace;font-size:9px;color:#5a6a7a;">' + escapeHtml(m.machine_id) + '</small></td>' +
                    '<td style="font-size:10px;">' + escapeHtml(m.version || '-') + '</td>' +
        ...
        }).catch(function() { el.innerHTML = riskCard + '<div class="text-center text-muted py-3">' + t('ui.loadErrX') + '</div>'; });
...
function doGlobalSearch() {
    ...
    fetch('/api/search?q=' + encodeURIComponent(q)).then(r => r.json()).then(d => {
        ...
        if (d.alerts && d.alerts.length) {
            ...
            html += d.alerts.map(a => '<div style="padding:3px 6px;border-bottom:1px solid #1e2a3a;font-size:11px;"><a href="#" onclick="event.preventDefault();closeGlobalSearch();showView(\'threats\')" style="color:#d0d8e0;">[' + escapeHtml(a.severity || '?') + '] ' + escapeHtml(a.rule_id || '?') + '</a> <small style="color:#8892a4;">' + escapeHtml(a.hostname || a.machine_id) + ' · ' + escapeHtml((a.description || '').substring(0, 50)) + '</small></div>').join('');
        }
        if (d.events && d.events.length) {
            html += '<div style="color:#88dd99;font-size:11px;margin-top:6px;">📋 EVENTS (' + d.events.length + ')</div>';
            html += d.events.map(e => '<div style="padding:3px 6px;border-bottom:1px solid #1e2a3a;font-size:11px;color:#8892a4;">' + escapeHtml(e.hostname || e.machine_id) + ' · ' + escapeHtml(e.subtype || '') + ' · ' + escapeHtml((e.description || '').substring(0, 60)) + '</div>').join('');
        }
...
document.addEventListener('keydown', function(ev) {
    if ((ev.ctrlKey || ev.metaKey) && ev.key.toLowerCase() === 'k') {
        ev.preventDefault();
        const ov = document.getElementById('globalSearchOverlay');
        if (ov && ov.style.display === 'block') closeGlobalSearch(); else openGlobalSearch();
    }
...
function showOnboarding() {
    fetch("/api/agent/onboarding").then(function(r){return r.json();}).then(function(d){
        if (d.error) { showToast("Loi: " + d.error); return; }
        showDetailModal("+ Cai dat Agent", '<div style="font-size:12px;">' +
            '<p>Copy file <b>GiamSatAgent.exe</b> vao may tram, chay:</p>' +
            '<pre style="background:#0a0f14;padding:10px;border-radius:6px;color:#88dd99;font-size:12px;">' + escapeHtml(d.install_cmd) + '</pre>' +
