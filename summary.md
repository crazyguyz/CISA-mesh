...
### Tool dọn dữ liệu vận hành + bước kiểm tra khi cài server (v5.0.8)
- **Vấn đề thực tế:** clone mới + cài lại vẫn thấy **syslog cũ, máy trạm cũ, cấu hình cũ** trên dashboard, vì dữ liệu nằm ở **PostgreSQL** (ngoài thư mục cài) và server mới đọc lại đúng DB đó theo `server\.env`.
- **`tools/reset_data.py` (mới):** xoá sạch dữ liệu vận hành nhưng **giữ schema** (chỉ DELETE/TRUNCATE dòng, không DROP bảng). Dry-run mặc định (chỉ báo cáo), `--apply` mới xoá, hỏi `YES` trừ khi `--yes`; `--mode all` (mặc định) / `--mode keep-config` (giữ `watchlist`, `group_policies`, `agent_groups`, `custom_dashboards`, `syslog_sources`); hỗ trợ **PG + SQLite**; PG dùng **1 câu TRUNCATE ... RESTART IDENTITY** (nhanh, reset sequence) + fallback DELETE từng bảng; **refresh materialized view** `mv_dashboard_stats`/`mv_dashboard_machines` để hết số liệu cache; **không đụng** `users.json`/`.env`/`logs/`/agent; cảnh báo bảng lạ do build mới hơn.
...
### Agent ngốn RAM + cache phình 13 GB + spam "LOG RESET" GIẢ (v5.0.8)
- **Hiện tượng (đo trên máy thật 2026-09-23, agent 6.0.0):** `GiamSatAgent.exe` working set **1,70 GB → 2,85 GB trong ~12 phút**, commit charge **22,7 GB**, CPU 1.640 s/3,7 h; `agent.log` **413 MB** với **73.365 dòng `LOG RESET DETECTED`** chỉ trong 10 MB cuối — kiểm tra: **100% là giả** (dạng *"dropped from 111386 to 111386"*, số giống nhau) rải trên **18 channel**; cache offline `%PROGRAMDATA%\GIAM-SAT\Agent\giamsat_cache.db` = **13,18 GB** (13,2 triệu dòng chưa gửi, `auto_vacuum=0` nên file không tự co); server: bảng `events` 114.967 dòng thì **95.910 dòng (83%) là `event_id='LOG_RESET'`** từ 3 máy trạm.
- **4 lỗi thật:**
  1. `agent/event_collector.py`: điều kiện `_newest_rec <= last_seen` coi **trạng thái bình thường** (channel không có event mới ⇒ số mới nhất **bằng** watermark) là "log bị xoá", rồi còn **đặt `last_seen = 0`** ⇒ mỗi 3 giây trên 18 channel: **1 cảnh báo HIGH giả + đọc lại toàn bộ channel** (~6 alert/giây, ~500k/ngày).
  2. Vòng "drain" đọc **toàn bộ** channel mỗi lần poll (tới 200 × 1024 = **204.800 đối tượng** win32 trong RAM, mỗi 3 giây) ⇒ cấp phát/giải phóng liên tục ⇒ RAM leo GB.
  3. `agent/encrypted_cache.py`: `verify_integrity()` chạy `SELECT id, data, chain_hash FROM log_cache ORDER BY id ASC` + **`fetchall()` toàn bộ bảng mỗi 30 giây** ⇒ nạp 13,2 triệu dòng vào Python + SHA-256 từng dòng — **đây chính là "bom" RAM**; `get_cache_size()` = `COUNT(*)` full scan bảng 13 GB.
  4. `flush_batch()` xoá **từng dòng + commit từng dòng** ⇒ không bao giờ đuổi kịp, và cache **không có giới hạn** kích thước nên phình vô hạn.
- **Fix:** `is_real_log_reset()` (chỉ tính reset khi **newest < watermark**, không tính khi bằng nhau) + throttle alert **1 lần/30 phút/channel** (vẫn luôn rewind watermark để không mù log); `should_stop_draining()` dừng vòng đọc khi gặp watermark/hết log + `MAX_EVENTS_PER_POLL=20.000` chặn trên bộ nhớ mỗi poll. `verify_integrity()` chỉ kiểm tra **cửa sổ mới nhất 5.000 dòng**, lấy dòng cũ nhất trong cửa sổ làm mốc (hết cảnh báo "tampered" giả sau khi flush xoá dòng đầu), **bỏ qua khi bảng không đổi**; `get_cache_size(limit)` đếm **có giới hạn**; `flush_batch` xoá **theo lô** (1 commit/lô); thêm **`MAX_CACHE_ROWS=200.000`** + `_enforce_cap()` tự cắt dòng cũ nhất (mỗi lần tối đa 50.000 dòng để transaction nhỏ) cho **cả** `encrypted_cache` và `log_cache`.
- **Log agent không có rotate (bug phụ):** tee `print()` trong `agent_core.py` mở và ghi append `agent.log` **không bao giờ rotate** → file lên **413 MB** mà không có gì thu hồi; nay có `rotate_log_if_needed()` (20 MB → `agent.log.1`, kiểm tra mỗi 200 dòng ghi) + `RotatingFileHandler` cho cả `agent_core.py` và `main.py`.
- **Tool mới `tools/agent_cache_maintenance.py`:** báo cáo kích thước/số dòng, `--trim N` (giữ N dòng mới nhất), `--purge`, `--vacuum` (thu hồi dung lượng) — dry-run mặc định, `--apply` mới ghi — dùng để xử lý các máy đang có cache phình to.
- **Test mới `tests/agent_memory_tests.py`:** **34/34 PASS** — reset giả (số bằng nhau) **không** bắn alert, reset thật vẫn phát hiện, dừng vòng đọc đúng lúc, `verify_integrity` giới hạn số dòng + phát hiện tamper + bỏ qua khi không đổi, count bị chặn, cap cache cắt đúng dòng cũ, flush theo lô gửi hết / giữ dòng chưa gửi.
### UI v5.0.8 — tab "Hướng dẫn" cho Quét IOC + MITRE ATT&CK bấm tên sự kiện ra đúng cảnh báo
- **Quét IOC — tab "Hướng dẫn" (mới):** nằm cạnh tab Quét IOC, gồm: (1) công dụng + phạm vi; (2) cấu trúc dữ liệu vào JSON (`{type,value,source,confidence}`) và CSV (kèm tên cột tương đương) + bảng field (bắt buộc / mặc định / công dụng); (3) bảng **loại IOC × bảng × cột được quét** lấy đúng theo `ioc_sweeper._scan_table`; (4) **danh sách 12 lệnh + công dụng từng lệnh** — `POST /api/ioc/sweep` (body JSON & upload file), `GET/POST /api/watchlist`, `/import`, `/<id>/toggle`, `DELETE /<id>`, `/push-intel`, retro sweeper tự động 60 phút, `threat_hunting.search_ioc`, chặn cảnh báo giả, xem cảnh báo thật — kèm quyền cần cho từng lệnh; (5) cấu trúc kết quả (`matches`, `results[:500]`, `stats`) + ý nghĩa từng field; (6) ghi chú & giới hạn (khớp **chuỗi con không phân biệt hoa/thường**, LIMIT 1.000 dòng mỗi cột, hash không được quét ở `network_traffic`…). Hàm `switchIocTab()` trong `dashboard.js` dùng lớp riêng `.tab-ioc-content` (không đụng handler `.tab-as-content` của view Tài sản).
- **MITRE ATT&CK — tên sự kiện bấm được:** trong modal chi tiết kỹ thuật, ô **Rule** giờ là **link** → `openMitreAlert(i)` mở **đúng cảnh báo đó** bằng modal chi tiết sẵn có (`showAlertRowDetail`: rule_id/rule_name/severity/host/time/description + `raw_data`); danh sách cảnh báo lưu theo **index** (`_mitreAlerts`) nên không nhúng JSON vào attribute `onclick` (an toàn quoting/XSS). Tiêu đề "MITRE Technique: `<id>`" cũng là link, và mỗi dòng có nút **🔎 Xem trong Đe dọa** → nhảy sang tab Threats và **lọc đúng rule** đó (`_threatRuleFilter`: khớp chính xác `rule_id`/`rule_name` trước, nếu không có mới khớp chứa; kèm chip lọc + nút ✕ xoá lọc). Đã kiểm chứng trên bản chạy thật: `ANOMALY-23565` → `/api/mitre/technique/ANOMALY-23565` trả đúng 1 alert (id 2243) và bộ lọc Đe dọa hiện đúng dòng đó.
- **i18n:** 76 khoá mới cho **cả vi và en**. **Test mới `tests/ui_wiring_tests.py` — 46/46 PASS:** kiểm tra wiring tĩnh (tab + 6 mục hướng dẫn, `switchIocTab`/`openThreatsByRule`/`clearThreatRuleFilter`, chỗ áp filter trong `loadThreats`, nút MITRE gọi đúng hàm) và **hợp đồng i18n** (không trùng khoá trong một bảng, vi/en khớp key, mọi khoá dùng trong UI đều có ở cả 2 bảng). Cú pháp 3 file JS đã kiểm bằng `node --check`.
...
| **v5.0.8** | **2026-09** | **Vệ sinh repo công khai + sửa lỗi build + tool dọn dữ liệu: bỏ IP nội bộ khỏi README/docstring (placeholder `100.x.y.z`/`192.168.1.50`), bỏ tên máy nội bộ khỏi changelog; đồng bộ `agent_version.txt` = `server/version.txt` = **6.0.0** (trước lệch 4.8.1/4.8.0 → vòng lặp update); `build-agent.cmd` bỏ default 3.9.3 hardcode (nay đọc `server\version.txt`), `build-agent.ps1 -Version` thành tuỳ chọn; `.gitignore` thêm `server/setup/start_server.bat`; giữ comment header `correlation_rules.yaml` khi ghi từ API/Sigma (`server/rules_io.py`); **`tools/reset_data.py`** xoá dữ liệu vận hành giữ schema (PG+SQLite, dry-run, `--mode keep-config`, refresh matview) + `setup_config.ps1` mục [9] tự phát hiện dữ liệu cũ và hỏi xoá; README thêm mục "Dữ liệu nằm ở đâu? / Cài lại sạch"** |
